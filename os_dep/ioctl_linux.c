	...
	union iwreq_data *wrqu, char *extra)
{
	...
	if (rtw_pwr_wakeup(padapter) == false)
		goto exit;
	if (wrqu->freq.m <= 1000) {
			    ...
			    union iwreq_data *wdata, char *extra)
{
	...
	*(extra + wrqu->length) = '\0';
	...
	if (strcmp(tmp[0], "status") == 0) {
		...
		EFUSE_GetEfuseDefinition(padapter, EFUSE_WIFI, TYPE_EFUSE_MAP_LEN , (PVOID)&max_available_len, false);
		...
		*extra = 0;
		...
		addr = 0;
		EFUSE_GetEfuseDefinition(padapter, EFUSE_WIFI, TYPE_EFUSE_REAL_CONTENT_LEN , (PVOID)&mapLen, false);
		...
		addr = 0;
		EFUSE_GetEfuseDefinition(padapter, EFUSE_BT, TYPE_EFUSE_REAL_CONTENT_LEN, (PVOID)&mapLen, false);
		RTW_INFO("Real content len = %d\n", mapLen);
#ifdef RTW_HALMAC
		...
		*extra = 0;
		...
		RTW_INFO("%s: cnts=%d\n", __FUNCTION__, cnts);
#ifndef RTW_HALMAC
		EFUSE_GetEfuseDefinition(padapter, EFUSE_BT, TYPE_EFUSE_MAP_LEN, (PVOID)&max_available_len, false);
		if ((addr + cnts) > max_available_len) {
			RTW_INFO("%s: addr(0x%X)+cnts(%d) parameter error!\n", __FUNCTION__, addr, cnts);
			err = -EFAULT;
			goto exit;
			   ...
			   union iwreq_data *wrqu, char *extra)
{
	...
	if (extra[0] == '0')
		rtw_disable_tdls_func(padapter, false);
	else if (extra[0] == '1')
		    ...
		    union iwreq_data *wrqu, char *extra)
{
	...
	if (rtw_is_tdls_enabled(padapter) == false) {
