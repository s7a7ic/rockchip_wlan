	...
	for (i = 0; i < dvobj->iface_nums; i++) {
		...
		if (check_fwstate(&iface->mlmepriv, WIFI_STATION_STATE) == true) {
			...
			if (check_fwstate(&iface->mlmepriv, _FW_LINKED) == true) {
				...
				#ifdef CONFIG_TDLS
				if (iface->tdlsinfo.link_established == true)
...
{
	return _rtw_mi_dequeue_writeport(padapter, false);
}
u8 rtw_mi_buddy_dequeue_writeport(_adapter *padapter)
{
	return _rtw_mi_dequeue_writeport(padapter, true);
}
#endif
...
void rtw_mi_ap_acdata_control(_adapter *padapter, u8 power_mode)
{
	...
	_rtw_mi_process(padapter, false, &in_data, _rtw_mi_ap_acdata_control);
}
void rtw_mi_buddy_ap_acdata_control(_adapter *padapter, u8 power_mode)
{
	u8 in_data = power_mode;

	_rtw_mi_process(padapter, true, &in_data, _rtw_mi_ap_acdata_control);
}
