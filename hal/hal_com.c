 ...
 * chk_type: true means to check enable, if 0x690 & bit1 (for 8051), WOW enable successful.
 *									If 0x1C7 == 0 (for 3081), WOW enable successful.
 *		     false means to check disable, if 0x690 & bit1 (for 8051), WOW disable fail.
...
#ifdef CONFIG_IPV6
		if (ppwrpriv->wowlan_ns_offload_en == true) {
				  ...
				  RSVDPAGE_LOC *rsvd_page_loc)
{
	...
	if (pwrctl->wowlan_pno_enable == false) {
		...
		*page_num += CurtPktPageNum;
		...
		if (pwrctl->wowlan_ns_offload_en == true) {
...
static void rtw_hal_set_lps_pg_info_rsvd_page(_adapter *adapter)
{
	...
	if (pwrpriv->wowlan_mode == true || pwrpriv->wowlan_ap_mode == true)
		drv_rsvdpage_num = rtw_hal_get_txbuff_rsvd_page_num(adapter, true);
	else
		drv_rsvdpage_num = rtw_hal_get_txbuff_rsvd_page_num(adapter, false);
...
#ifdef CONFIG_BT_COEXIST
	if (pwrctl->wowlan_mode == false ||
		pwrctl->wowlan_in_resume == true) {
	...
	if (precvpriv->store_law_data_flag) {
		...
		if ((dframe_type == WIFI_DATA_TYPE) || (dframe_type == WIFI_QOS_DATA_TYPE) || (padapter->registrypriv.mp_mode == 1)) {
			...
			if (psta) {
				...
				if ((_rtw_memcmp(psta->cmn.mac_addr, bc_addr, ETH_ALEN) != true) || (padapter->registrypriv.mp_mode == 1)) {
...
void dm_DynamicUsbTxAgg(_adapter *padapter, u8 from_timer)
{
	...
	if (IS_HARDWARE_TYPE_8821U(padapter)) { /* || IS_HARDWARE_TYPE_8192EU(padapter)) */
		...
		if ((pHalData->rxagg_mode == RX_AGG_USB) && (check_fwstate(pmlmepriv, _FW_LINKED) == true)) {
		...
		for (i = 0; i < pdvobjpriv->iface_nums; i++) {
			...
			if (rtw_linked_check(iface) == true) {
